        ...
        If Not IsV2 Then
            ...
            For Each line As String In hdiffFiles
                ...
                Catch ex As Exception
                    ...
                    Dim result As DialogResult = MessageBox.Show("合并时出现错误：" & ex.Message & vbCrLf & vbCrLf & "是否忽略并继续？", "警告：", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation)
    ...
    Private Sub 删除文件(文件路径 As String, 文件列表 As List(Of String))
        For Each df As String In 文件列表
            ...
            Catch ex As Exception
                ...
                Dim result As DialogResult = MessageBox.Show("删除文件时出现错误：" & ex.Message & vbCrLf & vbCrLf & "是否忽略并继续？", "警告：", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation)
                If result = DialogResult.No Then
                    Throw ex
                End If
            End Try
    ...
    Private Sub 移动文件(源路径 As String, 目标路径 As String)
        For Each 源文件 As String In Directory.GetFiles(源路径, "*", SearchOption.AllDirectories)
            ...
            Catch ex As Exception
                ...
                Dim result As DialogResult = MessageBox.Show("移动文件时出现错误：" & ex.Message & vbCrLf & vbCrLf & "是否忽略并继续？", "警告：", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation)
                If result = DialogResult.No Then
                    Throw ex
                End If
            End Try
